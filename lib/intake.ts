...
export function buildNativeIntakeAutomationPayload(payload: IntakePayload, options: { submissionId: string; caseId: string }) {
  const requiredDocumentCodes = getRequiredDocumentCodes(payload.caseType, payload.incomeProfile.borrowerProfiles);

  return {
    submissionId: options.submissionId,
    caseId: options.caseId,
    leadName: payload.applicant.fullName.trim(),
    spouseName: payload.coApplicant.hasCoApplicant ? payload.coApplicant.fullName?.trim() || '' : '',
    ...
    borrowerProfiles: payload.incomeProfile.borrowerProfiles,
    bankTargets: [],
    notes: payload.notes || '',
    source: 'native-intake',
    applicant: {
      fullName: payload.applicant.fullName.trim(),
      idNumber: payload.applicant.idNumber?.trim() || '',
      birthDate: payload.applicant.birthDate || '',
      maritalStatus: payload.applicant.maritalStatus || '',
    },
    coApplicant: {
      hasCoApplicant: payload.coApplicant.hasCoApplicant,
      fullName: payload.coApplicant.fullName?.trim() || '',
      idNumber: payload.coApplicant.idNumber?.trim() || '',
      birthDate: payload.coApplicant.birthDate || '',
    ...
    contact: {
      ...
      bestTimeToReach: payload.contact.bestTimeToReach || '',
    },
    financialProfile: {
      borrowerProfiles: payload.incomeProfile.borrowerProfiles,
      monthlyNetIncome: payload.incomeProfile.monthlyNetIncome || '',
      additionalIncome: payload.incomeProfile.additionalIncome || '',
      employmentNotes: payload.incomeProfile.employmentNotes || '',
      hasExistingLoans: payload.liabilities.hasExistingLoans,
      monthlyLoanRepayments: payload.liabilities.monthlyLoanRepayments || '',
      liabilityNotes: payload.liabilities.liabilityNotes || '',
    },
    property: {
      purchasePrice: payload.property.purchasePrice || '',
      requestedMortgageAmount: payload.property.requestedMortgageAmount || '',
      propertyCity: payload.property.propertyCity || '',
      propertyValueEstimate: payload.property.propertyValueEstimate || '',
      equityAvailable: payload.property.equityAvailable || '',
      timeline: payload.property.timeline || '',
    },
    consent: {
      privacyAccepted: payload.consent.privacyAccepted,
      advisorAuthorizationAccepted: payload.consent.advisorAuthorizationAccepted,
      accuracyConfirmed: payload.consent.accuracyConfirmed,
    },
    requiredDocumentCodes,
    intakeSummary: summarizeIntakeForNotes(payload),
    officeNotification: {
      channel: 'whatsapp',
      text: `ליד חדש נכנס ל-KeyPoint עבור ${payload.applicant.fullName.trim()} — נא לבדוק, לאשר ולפתוח את התיק.`,
    },
  };
}
